...
use crate::v1::ast::{self, AssignOp, BinOp, FuncKind, Item, TypeExpr, UnOp};
...
pub fn compile(src: &str, opts: &CompileOptions) -> CResult<Program> {
    let contract = crate::v1::parser::parse(src)?;
    let mut c = Checker {
        opts,
        consts: BTreeMap::new(),
        states: Vec::new(),
...
impl<'o> Checker<'o> {
    ...
    fn program(&mut self, contract: ast::Contract) -> CResult<Program> {
        ...
        // Frozen: version 1 programs carry no version 2 sections.
        let mut program = Program::empty(1, &contract.name);
        program.states = self.states.clone();
        program.events = self.events.clone();
